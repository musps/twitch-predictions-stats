...
const resolvers = {
  Mutation: {
    requestUser: async (_, { name: _name }, context) => {
      ...
      const checkChannel = await getChannel(name)

      if (checkChannel) {
        return {
          ...response,
          success: false,
          message: 'Channel already in database!',
        }
      }

      const upsert = await addRequestUser(name)
  ...
  Query: {
    ...
    topChannels: (_, args, context) => {
      return cacheReducer('topChannels', null, async () => {
        const data = await getTopChannels(23)
