      ...
      if (checkChannel) {
        ...
        if (flushCache) {
          const removesCachesResponse = await removeCaches([
            buildCacheObject('channel', { login: channel.login }),
          ])
        }
  ...
  return new Promise(async (resolve, reject) => {
    try {
      ...
      for (let channel of channels) {
        const response = await updateChannelPredictions(channel.login).catch(
          console.log
        )
        ...
        if (response && response.nb > 0) {
          ...
          removeCachesPattern.push(
            buildCacheObject('channel.stats', { login: channel.login })
          )
